...
def copy_tracks(db1,db2,track_ids):
    for id in track_ids:
        ...
        ids, filenames, x, y, text, layer = np.asarray([[m.image.id, m.image.filename, m.x, m.y, m.text, m.image.layer] for m in track.markers]).T
        db2.setMarkers(image=[db2.getImage(i) for i in ids], x=x,y=y,type=type, track=new_track, text=text)

if __name__ == "__main__":
    db1 = clickpoints.DataFile("/home/birdflight/Desktop/252Horizon.cdb")
    db2 = clickpoints.DataFile("/home/birdflight/Desktop/252_Tracked_LOG21.cdb")
