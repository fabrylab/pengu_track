...
def horizontal_equalisation(image, horizonmarkers, f, sensor_size, h=1, markers=[], max_dist=None):
    ...
    image = transform.rotate(image, np.arctan(m) * 180 / np.pi)#, center=[X/2., t+m * X / 2.])
    ...
    Phi = phi([x_h, y_h], [X_s, Y_s], [Y, X], f)#np.arctan((t * (2 / Y) - 1.) * (Y_s / 2. / f))*-1
    ...
    def warp(xy):
        ...
        # x_senkrecht = [0, 0, 1]
        # my_cross = np.cross(coord.T, x_s)
        # print("my shape", (my_cross * x_senkrecht).T.shape)
        # theta = np.arccos(np.sum((my_cross * x_senkrecht).T
        #                          , axis=0) / (np.linalg.norm(my_cross, axis=0) * 1)) * np.sign(xx_)  # * np.sign(np.tan(phi_)*h-yy_)
        # theta = np.arccos(np.cross(coord.T, x_s)[0])/(coord_norm*x_s_norm*np.sin(alpha)) * np.sign(xx) #* np.sign(np.tan(phi_)*h-yy_)
    ...
    if len(markers) == 0:
        ...
        if len(image.shape) == 3:
            # split the image in colors and perform interpolation
            return np.asarray([img_as_uint(map_coordinates(i, grid[:, ::-1], order=0)).reshape((X, Y))[::-1] for i in image.T]).T,\
...
for mask in db.getMasks(frame=frame):
    ...
    # eq = img_as_uint(eq)
    # plt.figure()
    plt.imshow(((eq/np.amax(eq))*n_regions).astype(int))
    # plt.figure()
    # plt.imshow(eq2)
    plt.show()
